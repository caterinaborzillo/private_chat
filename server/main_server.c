...
void* ricezione_invio_messaggi(void* sock_id) {  
        ...
        recv_mess_size = recvfrom(sock_udp, buf, sizeof(buf), 0, (struct sockaddr*) &client_address_recv, (unsigned int*)&sockaddr_len);
        ...
        if (!strcmp(buf,"quit\n")) {
            ...
            if (DEBUG) fprintf(stderr, "Aggiunto indirizzo del client alla lista degli indirizzi.");

            ret = sendto(sock_udp, buf, sizeof(buf), 0, (const struct sockaddr*) &client_address_recv, sockaddr_len);
            
            if (ret != sizeof(buf)) handle_error("Errore, messaggio troppo lungo\n");
            memset(buf, 0, sizeof(buf));
                ...
                if (z==0) {
                    ...
                    if (l==0) {
                        ...
                        //if (DEBUG) fprintf(stderr, "indirizzo a cui ho inviato il messaggio: %d \n", (l->c_addr).sin_addr.s_addr);
                            ...
                            if (DEBUG) fprintf(stderr, "messaggio non inviato. \n");
                        }
                }
                z++;
                p = strtok (NULL, ":"); 
...
void udp_server_connection() {    
    if (DEBUG) fprintf(stderr, "--- AVVIO SERVER SU SOCKET UDP ---\n");
...
// serial server means: it accepts one connection at time
void serialServer(int server_desc) {
    int i = 0;
    if (DEBUG) fprintf(stderr, "Inizio serial server (per login/registrazione)\n\n");
    ...
    while (1) {
        ...
        if (DEBUG) fprintf(stderr, "I'm ready to accept new connections..\n\n");
        ...
        if (DEBUG) fprintf(stderr, "Accepted new connection. \n\n");
...
void connection_handler(int client_desc, struct sockaddr_in* client_addr) {
    ...
    memset(buf, 0, buf_len);
        ...
        if (DEBUG) fprintf(stderr, "Questo user vuole fare la/il: %s \n", buf);
...
// funzione che stampa tutti gli utenti online
void stampa_utenti_udp(ListAddress* curr_user, int socket) {
    int i=0;
    int sockaddr_len = 0;
...
void stampa_utenti(char* curr_user, int socket_desc, char* buf) {
    ...
    memset(buf, 0, buf_len);
    sprintf(buf, "Per specificare il destinatario del messaggio scrivilo come prima parola seguito dai due punti ':' es. paolo: ciao paolo! Ora ti invierò la tua password per avere una conferma della tua identità. \n");
    send_tcp_message(buf, socket_desc);
    memset(buf, 0, buf_len);
   return;
}

// ritorna 1 se è presente ritona 0 se il nome non è presente (cioè non si era registrato)
...
void registrazione(int socket_desc) {
        ...
        if (DEBUG) fprintf(stderr, "Dati del nuovo user: %s %s %s\n", u->nome, u->username, u->password);
...
void login(int socket_desc) {
        ...
        if (database_research(username,password)){
            memset(buf, 0, buf_len);
            sprintf(buf, "Login effettuato! Per iniziare premi 'invio'. Ricorda: se vuoi disconnetterti digita 'quit'. \n");
            if (DEBUG) fprintf(stderr, "Login effettuato.\n");
            send_tcp_message(buf, socket_desc);
...
void signalHandler(int sig){
    ret = close(sock_udp);
    if (ret) handle_error("Cannot close socket UDP");
    ret = close(socket_desc);
    if (ret) handle_error("Cannot close socket TCP");
    free(mittente);
    free(dest);
...
int main() {
    ...
    // stampa del database attuale 
    for (int i=0; i<n; i++) {
            if (DEBUG) fprintf(stderr, "user[%d]-> username: %s\nuser[%d]-> nome: %s\nuser[%d]-> password: %s\n", i, user[i]->username, i, user[i]->nome, i, user[i]->password);
    }
